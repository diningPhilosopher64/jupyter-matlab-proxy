...
export class ConvertAction extends BaseAction {
    ...
    public async execute (data: any, comm: ICommunicationChannel): Promise<void> {
        if (
            !('action' in data) ||
      data.action !== 'convert' ||
      !('ipynbFilePath' in data) ||
      !('mlxFilePath' in data)
        ) {
            console.error(
                'action, ipynbFilePath or mlxFilePath missing in data:',
                data
            );
            return;
        }
        ...
        Notification.promise(ConvertAction.blockingPromise.promise, {
            pending: {
                message: ((): string => {
                    if (
                        this.sendConvertRequest(data.ipynbFilePath, data.mlxFilePath, comm)
                    ) {
                    ...
                    setTimeout(() => {
                        ...
                        if (ConvertAction.blockingPromise) {
                            ConvertAction.blockingPromise.reject({
