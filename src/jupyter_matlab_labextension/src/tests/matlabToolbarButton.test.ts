  ...
  describe('matlab_browser_button', () => {
    ...
    describe('insertButton', () => {
      test('should insert button when kernel is MATLAB Kernel', async () => {
        ...
        // Assert
        expect(panel.toolbar!.insertItem).toHaveBeenCalledWith(10, 'matlabToolbarButton', button);
      });
      ...
      test('should wait for session context to be ready before checking kernel', async () => {
        // Arrange
        const readyPromise = new Promise<void>(resolve => setTimeout(resolve, 10));
        const panel = {
          sessionContext: {
            ready: readyPromise,
    ...
    describe('MatlabToolbarButtonExtension', () => {
      ...
      test('button onClick should open MATLAB in a new tab', () => {
        ...
        // Act
        extension.createNew(panel as unknown as NotebookPanel, context as unknown as DocumentRegistry.IContext<INotebookModel>);
        // Manually call the onClick handler
        capturedOnClick();
   
        // Assert
        expect(window.open).toHaveBeenCalledWith('http://localhost:8888/matlab', '_blank');
      });
      ...
      test('should call insertButton with panel and button', () => {
        ...
        // Act
        const button = extension.createNew(panel as unknown as NotebookPanel, context as unknown as DocumentRegistry.IContext<INotebookModel>);
    ...
    describe('matlabToolbarButtonPlugin', () => {
      ...
      test('should create a MatlabToolbarButtonExtension instance on activation', () => {
        ...
        const extensionArg = (app.docRegistry!.addWidgetExtension as jest.Mock).mock.calls[0][1];
