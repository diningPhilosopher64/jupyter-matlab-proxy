...
export class NotebookInfo {
        ...
        if (!this._panel) {
            ...
            return new Promise((resolve) => {
                if (this._panel!.sessionContext.session?.kernel?.status === 'idle') {
                    ...
                    this._panel!.sessionContext.session?.kernel?.statusChanged.connect(onStatusChanged);
                }
            });
        }
    }
    ...
    async update (panel: NotebookPanel | null) : Promise<void> {
        if (panel) {
            this._panel = panel;
            this._isMatlabNotebook = panel.context.model.metadata.kernelspec?.language === 'matlab';
