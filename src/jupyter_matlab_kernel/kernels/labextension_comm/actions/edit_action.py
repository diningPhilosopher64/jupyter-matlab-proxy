...
class EditAction(ActionCommand):
    ...
    async def execute(self, comm, data):
        ...
        while True:
            ...
            # Keep send eval execution requests till client type is set
            try:
                eval_response = await self.kernel.mwi_comm_helper.send_eval_request_to_matlab(client_type_code)
                self.log.debug(f"The output received for client type request {eval_response}")
                if eval_response['isError']:
                    self.log.error(f"Failed to check client type :{eval_response['responseStr']}")
                ...
                if time_taken > time_out:
                    err = TimeoutError(f"Failed to set client type within {time_out} seconds.")
                    self.log.error(err)
                    comm.send({
                        'action': ActionTypes.EDIT.value, 
                        "error": str(err)
        ...
        try:
            ...
            if eval_response['isError']:
                self.log.error(f"Failed to edit file with error:{eval_response['response_str']}")
                comm.send({
                    "action": ActionTypes.EDIT.value,
